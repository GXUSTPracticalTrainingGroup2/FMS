...
import com.ss.vv.ss.vo.TestVoFont;
/** 
* 
* @author bingoWu 
...
/*
 * Postman使用方法：
 * http://localhost:8080/auto/test/addTest?testName=水&info=值&other=你
 * 
 * */
	...
	public WebResponse addTest(HttpServletRequest request, HttpServletResponse response, HttpSession session, String testName, String info, String other) {
		...
		if (testName.length() > 255 || info.length() > 65535 || other.length() > 255) {
			statusMsg = " 参数长度过长错误！！！";
			statusCode = 201;
			return webResponse.getWebResponse(statusCode, statusMsg, data);
		}
 ...
 * */
	...
	public WebResponse editTest(HttpServletRequest request, HttpServletResponse response, HttpSession session, String testId, @RequestParam(required = false) String testName, @RequestParam(required = false) String info, @RequestParam(required = false) String other) {
		...
		TestVo testVo = this.testService.getById(testIdNumeri);
		Test test = new Test();
 ...
 * 通过id查询test表的数据
 * Postman使用方法：
 * http://localhost:8080/auto/test/getTestById?testId=1
 * 
 * 
 * */
	...
	public WebResponse getTestById(String testId) {
		...
		TestVo testVo = this.testService.getById(testIdNumNumeri);
 ...
 * 
 * 
 * http://localhost:8080/auto/test/getOneTest?
 * */
	...
	public WebResponse getOneTest(@RequestParam(defaultValue = "正常", required = false) String tbStatus) {
		...
		TestVo testVo = this.testService.getOne(condition);
		Object data = null;
		String statusMsg = "";
		if (testVo != null && testVo.getTestId() > 0) {
			data = testVo;
	...
	public WebResponse getTestList(HttpServletRequest request, HttpServletResponse response, HttpSession session,
		...
		List<TestVo> list = this.testService.getList(condition, pageNo, pageSize, order, field);
		Map<Object, Object> map = new HashMap<Object, Object>();
		map.put("total", count);
		int size = list.size();
		if (size > 0) {
			List<TestVoFont> listFont = new ArrayList<TestVoFont>();
			TestVo vo;
			TestVoFont voFont = new TestVoFont(); 
			for (int i = 0; i < size; i++) {
				vo = list.get(i);
				BeanUtils.copyProperties(vo, voFont);
				listFont.add(voFont);
				voFont = new TestVoFont();
			}
